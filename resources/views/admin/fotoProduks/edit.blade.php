...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.fotoProduk.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.foto-produks.update", [$fotoProduk->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('produk_unggulan') ? 'is-invalid' : '' }}" name="produk_unggulan_id" id="produk_unggulan_id" required>
                    @foreach($produk_unggulans as $id => $produk_unggulan)
                        <option value="{{ $id }}" {{ (old('produk_unggulan_id') ? old('produk_unggulan_id') : $fotoProduk->produk_unggulan->id ?? '') == $id ? 'selected' : '' }}>{{ $produk_unggulan }}</option>
                    @endforeach
                </select>
