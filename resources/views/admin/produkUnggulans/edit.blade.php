...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.produkUnggulan.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.produk-unggulans.update", [$produkUnggulan->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nama">{{ trans('cruds.produkUnggulan.fields.nama') }}</label>
                <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" name="nama" id="nama" value="{{ old('nama', $produkUnggulan->nama) }}" required>
                @if($errors->has('nama'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('deskripsi') ? 'is-invalid' : '' }}" type="text" name="deskripsi" id="deskripsi" value="{{ old('deskripsi', $produkUnggulan->deskripsi) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('usaha') ? 'is-invalid' : '' }}" name="usaha_id" id="usaha_id" required>
                    @foreach($usahas as $id => $usaha)
                        <option value="{{ $id }}" {{ (old('usaha_id') ? old('usaha_id') : $produkUnggulan->usaha->id ?? '') == $id ? 'selected' : '' }}>{{ $usaha }}</option>
                    @endforeach
                </select>
                @if($errors->has('usaha'))
