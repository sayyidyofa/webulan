...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.mediaSosial.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.media-sosials.update", [$mediaSosial->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="link_accname">{{ trans('cruds.mediaSosial.fields.link_accname') }}</label>
                <input class="form-control {{ $errors->has('link_accname') ? 'is-invalid' : '' }}" type="text" name="link_accname" id="link_accname" value="{{ old('link_accname', $mediaSosial->link_accname) }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\MediaSosial::VENDOR_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('vendor') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="vendor_{{ $key }}" name="vendor" value="{{ $key }}" {{ old('vendor', $mediaSosial->vendor) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('usaha') ? 'is-invalid' : '' }}" name="usaha_id" id="usaha_id" required>
                    @foreach($usahas as $id => $usaha)
                        <option value="{{ $id }}" {{ (old('usaha_id') ? old('usaha_id') : $mediaSosial->usaha->id ?? '') == $id ? 'selected' : '' }}>{{ $usaha }}</option>
                    @endforeach
                </select>
                @if($errors->has('usaha'))
